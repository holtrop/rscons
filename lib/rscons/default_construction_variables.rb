module Rscons
  ...
  DEFAULT_CONSTRUCTION_VARIABLES = {
    ...
    "SHDC" => "${DC}",
    "SHDC:-o" => "${DC:-o}",
    "SHDCCMD" => %w[${SHDC} -c ${SHDC:-o} ${_TARGET} ${DDEPGEN} ${INCPREFIX}${D_IMPORT_PATH} ${SHDFLAGS} ${_SOURCES}],
    "SHDCCMD:direct" => %w[${SHDC} ${SHDC:-o} ${_TARGET} ${DDEPGEN} ${INCPREFIX}${D_IMPORT_PATH} ${SHDFLAGS} ${SHLDFLAGS} ${_SOURCES} ${LIBDIRPREFIX}${LIBPATH} ${LIBLINKPREFIX}${LIBS}],
    "SHDFLAGS" => %w[${DFLAGS} ${SHDPICFLAGS}],
    "SHDPICFLAGS" => pic_flags,
    "SHLD" => nil,
    "SHLD:-o" => "${LD:-o}",
    "SHLDCMD" => %w[${SHLD} ${SHLD:-o} ${_TARGET} ${SHLDFLAGS} ${_SOURCES} ${SHLIBDIRPREFIX}${LIBPATH} ${SHLIBLINKPREFIX}${LIBS}],
